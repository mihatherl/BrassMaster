...
> **Superseded by `handover.md` (2026-08-30).** Kept because everything here
> about the app *outside* the course schema still stands: the two halves of
> the site, the i18n guard and its eight locales, the honesty rulings, the
> sessions store, and the road to Google Play. The course-schema sections —
> `advance`, `pinned`, tempo bands — were replaced by the axes build on
> 2026-08-29; `level-axes-plan.md` is their record. The patch notes below
> were written as that build landed and are left as they were.
