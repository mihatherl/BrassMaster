...
> **Superseded by `handover.md`**, which covers the session of 2026-08-21/22:
> divisi, the corpus reaching 64 of 64 heard, and the calibration rework. This
> file is kept for the corpus-pipeline work it records — the converter, the
> collections, the tempo ruling and the reading floor — which still stands. Its
> figures (v2.28.0, 1,332 tests) are stale.
